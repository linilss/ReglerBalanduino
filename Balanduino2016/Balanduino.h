...
static const uint8_t eepromVersion = 99;	// EEPROM version - used to restore the EEPROM values if the configuration struct have changed
...
static uint32_t pidTimer_us;	// Timer used for the PID loop
...
static double scale(double input, double inputMin, double inputMax,
...
static uint8_t i2cWrite(uint8_t registerAddress, uint8_t data,
			bool sendStop);
static uint8_t i2cWrite(uint8_t registerAddress, uint8_t * data,
			uint8_t length, bool sendStop);
static uint8_t i2cRead(uint8_t registerAddress, uint8_t * data,
		       uint8_t nbytes);
...
static void testMotorSpeed(double *leftSpeed, double *rightSpeed,
			   double leftScaler, double rightScaler);
...
static bool checkMinMax(int16_t * array, uint8_t length,
			int16_t maxDifference);
